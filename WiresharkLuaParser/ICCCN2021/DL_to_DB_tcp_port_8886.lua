...
do
    ...
    -- register protocol
    local proto_name = "DBC"
    local proto_desc = "Database Collection"
    ...
    local function dissector(buf, pkt, root)
        ...
        if buf_len < proto_fields_len then
            ...
            pkt.cols.protocol = "DB Collection"
            pkt.cols.info:set("Digested Telemetry Data Records")

            local rt = root:add(proto_DT, buf)
            rt:set_text("Digested Data Vectors")
