...
do
    ...
    local int_d_ingress_time = ProtoField.uint64("INT.ingress_time", "ingress_time", base.DEC)
    local int_d_ingress_time_ovs_pof = ProtoField.uint64("INT.ingress_time", "ingress_time (us)", base.DEC)
    local int_d_ingress_time_tofino = ProtoField.uint64("INT.ingress_time", "ingress_time (ns)", base.DEC)
    ...
    local int_d_hop_latency_tofino = ProtoField.uint32("INT.hop_latency", "hop_latency (ns)", base.DEC)

    local int_d_bandwidth = ProtoField.float("INT.bandwidth", "bandwidth (Gbps)", base.DEC)
    ...
    INT_proto.fields = {
        ...
        int_d_ingress_time,
        int_d_ingress_time_ovs_pof,
        int_d_ingress_time_tofino,

        int_d_hop_latency,
        int_d_hop_latency_ovs_pof,
        int_d_hop_latency_tofino,
    ...
    local function dissector(buf, pkt, root)
        ...
        if (bit32.band(v_int_h_mapInfo:uint(), bit32.lshift(bit_one, 0))) then      -- must contain device_id
        ...
        for i=1,hop do
            ...
            if (bit32.band(v_int_d_dpid:uint(), switch_mask) == 0x00) then   -- ovs-pof
                ...
                if (bit32.band(switch_mapInfo, bit32.lshift(bit_one, 3))) then
                    local v_int_d_ingress_time = buf(base, INT_d_ingress_time_len)
                    t:add(int_d_ingress_time_tofino, v_int_d_ingress_time)
                    base = base + INT_d_ingress_time_len
                end

                if (bit32.band(switch_mapInfo, bit32.lshift(bit_one, 4))) then
                    local v_int_d_hop_latency = buf(base, INT_d_hop_latency_len)
                    t:add(int_d_hop_latency_tofino, v_int_d_hop_latency)
                    base = base + INT_d_hop_latency_len
                end
