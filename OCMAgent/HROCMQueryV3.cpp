 ...
 * @author tsf
 * @date 20-8-31.
 * @ref https://github.com/kongjiawei/OCM/blob/928febbd1cba3f7b3d5ba29a4904cc9a1fd0972d/HROCMQueryV3_2019_12_27.cpp
 */
...
//全局变量
float i_start = 191.3, i_grid = 0.05, slice = 0.0003125, fsf = 191.25; //THz,FSF，不能改
int Flag = 0;//1 表示开始， 0表示结束
int MAXRECV = 5;
//float start_fre = 192.3;

double start_fre = 192.60;     // tsf, 192.6-0.05/2 = 192.575，可以改，这里应该是边缘频率
//float window = 0.05;
double window = 0.05;  // tsf
double expected_request_slice = slice;   // tsf，将多个slice相加合成一个expected_request_slice
int expected_slice_num = (int)ceil(window / expected_request_slice);
...
int LOOP = 1;
while (LOOP) {
char *buffer = (char*)malloc(MAXRECV * sizeof(double)); //
int ret = recv(sClient, buffer, 1024, 0);
now = time(&now);
printf("tsf, time: %s", ctime(&now));
if (ret <= 0) {
...
buffer[ret] = '\0';
printf("tsf, recv_bytes: %d, recvdata: %s", strlen(buffer), buffer);

const char *s = " "; //以空格进行划分
char *token;
if (strlen(buffer) > 2) {
token = strtok(buffer, s); //strtok进行字符的划分, tsf
...
token = strtok(NULL, s);
expected_request_slice = atof(token);
expected_slice_num = (int)ceil(window / expected_request_slice);

Flag = 1;
printf("tsf, start_fre: %lf, window: %lf, fixed_slice: %lf, expected_slice: %lf, slice_num: %d.\n",
start_fre, window, slice, expected_request_slice, expected_slice_num);
}
else {
Flag = 0;
}
...
static double dBm_to_Pmw(double dBm) {  // tsf
    double Pmw = pow(10.0, dBm / 10.0);
    return Pmw;
}

static double Pmw_to_dBm(double Pmw) { // tsf
...
{
    //ofstream outfile("position_11.txt"); //输出文件
    //if (!outfile) {
    ...
    while (true) {
        if (Flag)
        {
            ...
            std::vector<double>send2Client((int)round(window / slice)); //乘2是因为同时存放fre, power, osnr

            std::vector<double> request_slice_win_data_to_ocm_collector(expected_slice_num);  // tsf，求和数据后发送
            int slice_win_idx = 0;
            double slice_win_sum = 0;
            int slice_win_ceil = 0;
            int slice_win_num = (int) (send2Client.size() / expected_slice_num);  // 每个expected_slice里应该放几个slice里的元素，最后一个可能包含多一点
            //printf("tsf slice_win_num: %d\n", slice_win_num);

            //printf("tsf, Flag is true, I am here to collect data to send.\n");
            ...
            // scan, 别用scanosnr
            {
                ...
                if (Result == OCM_OK && pRDataDEV != NULL) {
                    OCM3_GMPWResult_t* pGMPWResult = OCM.getGMPWResult();
                    //for (unsigned int k = 0; k < pGMPWResult->GMPWVector.size(); ++k) {
                    //	myNode->power_slice[k] = pGMPWResult->GMPWVector[k].POWER / OCM3_PSCALE;
                    //	//printf("power density:%f\n", pThreadData->power_slice[k]);
                    //}

                    for (slice_win_idx = 0; slice_win_idx < expected_slice_num; slice_win_idx++) {  // tsf
                        for (int i = slice_win_idx * slice_win_num; i < (int)(window / slice); i++) {
                            //printf("index:%d\n", (int)round(index));
                            send2Client[i] = pGMPWResult->GMPWVector[(int)round(index)].POWER / OCM3_PSCALE;

                            if (slice_win_idx < expected_slice_num - 1) {
                                slice_win_ceil = (slice_win_idx + 1) * slice_win_num;
                            }
                            else {  // 最后一段可能多包括一些点数
                                slice_win_ceil = (int)(window / slice);
                            }

                            //printf("tsf ++ index: %d, slice_win_ceil: %d\n", i, slice_win_ceil);

                            if (i < slice_win_ceil) {
                                slice_win_sum += dBm_to_Pmw(send2Client[i]);
                                //printf("tsf ++ index: %d, slice_win_sum: %lf\n", i, slice_win_sum);
                            }
                            else {  // 计算汇聚后的dBm值
                                request_slice_win_data_to_ocm_collector[slice_win_idx] = Pmw_to_dBm(slice_win_sum);
                                slice_win_sum = 0;
                                break;
                            }

                            if (i == (int)(window / slice)-1) {    // 计算最后一个slice里汇聚后的dBm值。最后一个，不需要break
                                request_slice_win_data_to_ocm_collector[slice_win_idx] = Pmw_to_dBm(slice_win_sum);
                            }

                            //printf("tsf ++ index: %d, power: %f\n", i, send2Client[i]);
                            index++;
                        }
                        printf("tsf++ slice_win_idx: %d, power_sum: %f\n", slice_win_idx, request_slice_win_data_to_ocm_collector[slice_win_idx]);
                    }
            ...
            //for (int i = k; i < (int)(window / slice); i++) {
            //	//printf("index:%d\n", (int)round(index));
            ...
            int position = 0;    // tsf
            int needSend = sizeof(double) * request_slice_win_data_to_ocm_collector.size();
            char* sendbuffer = (char*)malloc(needSend);
            memcpy(sendbuffer, &request_slice_win_data_to_ocm_collector[0], needSend);
            while (position < needSend) {     //可能不能一次性发完所有数据
                int length = send(sClient, sendbuffer + position, needSend, 0);
                if (length < 0) {
                    printf("server Transmit Data Failed");
